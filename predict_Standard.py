...
weights_pth = "weights/resnet101_weights_Latest.pth"
...
def overlay_mask(image, predictions, orientation_img):
    ...
    if os.path.isfile(pth+f"/{txt_name}"):
        with open(pth+f"/{txt_name}", "r+") as f:
            f.truncate(0)
    for mask, color, conf in zip(masks, colors, confidence):
        ...
        with open(pth+f"/{txt_name}", "a") as f:
            f.write(f"{cntr[0]} { cntr[1]} {-angle} {conf} \n") 

    cv2.imwrite(pth+f"/{img_name}", orientation_img)
...
def getOrientations(pts, img):
    ...
    cv2.drawMarker(img, cntr,(255, 0, 0), markerType=cv2.MARKER_CROSS, markerSize=5, thickness=1 )
...
if __name__ == "__main__":
    global pth, img_name 
    pth = "datasets/real_test/Results"
